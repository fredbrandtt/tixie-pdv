...
/**
 * Rota proxy para evitar problemas de CORS ao buscar eventos
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[API] Proxy de eventos recebeu requisição. companyId:', requestData.companyId);
    
    // Validar os dados recebidos
    if (!requestData.companyId) {
      console.error('[API] Parâmetro inválido para busca de eventos');
      return NextResponse.json(
        { error: 'Parâmetro inválido. É necessário fornecer companyId.' },
        { status: 400 }
      );
    }
    
    // Construir a URL do webhook
    const webhookUrl = process.env.NEXT_PUBLIC_WEBHOOK_EVENTOS;
    if (!webhookUrl) {
      console.error('[API] URL do webhook de eventos não configurada');
      return NextResponse.json(
    ...
    console.error('[API] Erro no proxy de eventos:', error);
    return NextResponse.json(
      { error: 'Erro interno do servidor ao processar a requisição de eventos' },
      { status: 500 }
    );
  }
} 